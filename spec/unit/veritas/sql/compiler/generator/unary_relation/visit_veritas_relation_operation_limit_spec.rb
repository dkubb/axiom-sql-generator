...
describe Generator::UnaryRelation, '#visit_veritas_relation_operation_limit' do
  ...
  context 'when the relation is limited' do
    ...
    its(:to_s) { should eql('SELECT DISTINCT "users"."id", "users"."name", "users"."age" FROM (SELECT * FROM (SELECT * FROM (SELECT * FROM "users") AS "users" ORDER BY "users"."id", "users"."name", "users"."age") AS "users" LIMIT 2) AS "users" LIMIT 1') }
  end
  ...
  context 'when the relation is offset' do
    ...
    its(:to_s) { should eql('SELECT DISTINCT "users"."id", "users"."name", "users"."age" FROM (SELECT * FROM (SELECT * FROM (SELECT * FROM "users") AS "users" ORDER BY "users"."id", "users"."name", "users"."age") AS "users" OFFSET 1) AS "users" LIMIT 1') }
  end
end
