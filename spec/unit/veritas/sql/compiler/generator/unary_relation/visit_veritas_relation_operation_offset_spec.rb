...
describe Generator::UnaryRelation, '#visit_veritas_relation_operation_offset' do
  ...
  context 'when the relation is a projection' do
    ...
    its(:to_s) { should eql('SELECT DISTINCT "users"."id", "users"."name" FROM "users" ORDER BY "users"."id", "users"."name" OFFSET 1') }
  end
  ...
  context 'when the relation is a rename' do
    ...
    its(:to_s) { should eql('SELECT "users"."user_id", "users"."name", "users"."age" FROM (SELECT "users"."id" AS "user_id", "users"."name", "users"."age" FROM "users") AS "users" ORDER BY "users"."user_id", "users"."name", "users"."age" OFFSET 1') }
  end
  ...
  context 'when the relation is limited' do
    ...
    its(:to_s) { should eql('SELECT "users"."id", "users"."name", "users"."age" FROM (SELECT * FROM "users" ORDER BY "users"."id", "users"."name", "users"."age" LIMIT 2) AS "users" OFFSET 1') }
  end
  ...
  context 'when the relation is offset' do
    ...
    its(:to_s) { should eql('SELECT "users"."id", "users"."name", "users"."age" FROM (SELECT * FROM "users" ORDER BY "users"."id", "users"."name", "users"."age" OFFSET 1) AS "users" OFFSET 1') }
  end
end
