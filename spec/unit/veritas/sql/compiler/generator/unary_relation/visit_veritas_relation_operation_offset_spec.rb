...
describe Generator::UnaryRelation, '#visit_veritas_relation_operation_offset' do
  ...
  context 'when the relation is a rename' do
    ...
    its(:to_s) { should eql('SELECT DISTINCT "users"."user_id", "users"."name", "users"."age" FROM (SELECT * FROM (SELECT "users"."id" AS "user_id", "users"."name", "users"."age" FROM (SELECT * FROM "users") AS "users") AS "users" ORDER BY "users"."user_id", "users"."name", "users"."age") AS "users" OFFSET 1') }
  end
