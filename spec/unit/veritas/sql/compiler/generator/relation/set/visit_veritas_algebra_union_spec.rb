...
describe Generator::Relation::Set, '#visit_veritas_algebra_union' do
  ...
  context 'when the operand is a join' do
    ...
    its(:to_s)     { pending { should eql('(SELECT "id", "name", "age" FROM "users" NATURAL JOIN "users") UNION (SELECT "id", "name", "age" FROM "users" NATURAL JOIN "users")') } }
    its(:to_inner) { pending { should eql('(SELECT * FROM "users" NATURAL JOIN "users") UNION (SELECT * NATURAL JOIN "users")') } }
  end
