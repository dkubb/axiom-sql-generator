...
describe SQL::Generator::Relation::Unary, '#visit_veritas_relation_operation_offset' do
  ...
  context 'when the operand is a summarization' do
    ...
    context 'summarize by a subset of the operand header' do
      ...
      its(:to_s)        { should eql('SELECT "id", "name", COALESCE (COUNT ("id"), 0) AS "count" FROM "users" GROUP BY "id", "name" HAVING COUNT (*) > 0 ORDER BY "id", "name", "count" OFFSET 1')   }
      its(:to_subquery) { should eql('(SELECT "id", "name", COALESCE (COUNT ("id"), 0) AS "count" FROM "users" GROUP BY "id", "name" HAVING COUNT (*) > 0 ORDER BY "id", "name", "count" OFFSET 1)') }
    end
  end
