...
describe SQL::Generator::Relation::Unary, '#visit_veritas_relation_operation_limit' do
  ...
  context 'when the operand is a summarization' do
    context 'summarize per table dee' do
      ...
      its(:to_s)        { should eql('SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" ORDER BY "count" LIMIT 1')   }
      its(:to_subquery) { should eql('(SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" ORDER BY "count" LIMIT 1)') }
    end
    ...
    context 'summarize per table dum' do
      ...
      its(:to_s)        { should eql('SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING FALSE ORDER BY "count" LIMIT 1')   }
      its(:to_subquery) { should eql('(SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING FALSE ORDER BY "count" LIMIT 1)') }
    end
