...
describe SQL::Generator::Relation::Set, '#visit_veritas_algebra_union' do
  ...
  context 'when the operand is a summarization' do
    context 'summarize per table dee' do
      ...
      its(:to_s)        { should eql('(SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users") UNION (SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users")')   }
      its(:to_subquery) { should eql('((SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users") UNION (SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users"))') }
    end
    ...
    context 'summarize per table dum' do
      ...
      its(:to_s)        { should eql('(SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING FALSE) UNION (SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING FALSE)')   }
      its(:to_subquery) { should eql('((SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING FALSE) UNION (SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING FALSE))') }
    end
    ...
    context 'summarize by a subset of the operand header' do
      ...
      its(:to_s)        { should eql('(SELECT "id", "name", COALESCE (COUNT ("age"), 0) AS "count" FROM "users" GROUP BY "id", "name" HAVING COUNT (*) > 0) UNION (SELECT "id", "name", COALESCE (COUNT ("age"), 0) AS "count" FROM "users" GROUP BY "id", "name" HAVING COUNT (*) > 0)')   }
      its(:to_subquery) { should eql('((SELECT "id", "name", COALESCE (COUNT ("age"), 0) AS "count" FROM "users" GROUP BY "id", "name" HAVING COUNT (*) > 0) UNION (SELECT "id", "name", COALESCE (COUNT ("age"), 0) AS "count" FROM "users" GROUP BY "id", "name" HAVING COUNT (*) > 0))') }
    end
  end
