...
describe SQL::Generator::Relation::Set, '#visit_veritas_algebra_union' do
  ...
  context 'when the operand is a summarization' do
    ...
    context 'summarize per table dum' do
      ...
      its(:to_s)        { should eql('(SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING 1 = 0) UNION (SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING 1 = 0)')   }
      its(:to_subquery) { should eql('((SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING 1 = 0) UNION (SELECT COALESCE (COUNT ("id"), 0) AS "count" FROM "users" HAVING 1 = 0))') }
    end
